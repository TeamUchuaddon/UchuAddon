# Readme
**日本語**  
[English](https://github.com/TeamUchuaddon/UchuAddon/blob/main/README-ENGLISH.md)

---
# UchuAddon

**UchuAddon** は、**Nebula on the Ship** 向けに作られたアドオンです。  
新しい役職やゲーム要素を追加し、プレイ体験を拡張します。

---
# <span style="color: red; ">注意事項</span>
## 本アドオンはNebula on the Ship公式様とは関係ありません。本アドオンについて、本家Nebula on the Shipの公式サーバー及び報告フォームに報告しないようにお願いいたします。また、本アドオンを入れて起きた不具合はそのままNoS公式様に報告せず、本アドオンを未導入の状態でも発生する不具合であることを確認してから報告するようにお願いします。本アドオンについてのお問い合わせは、UchuAddonのゲーム内報告フォーム、[UchuAddon Wikiの報告フォーム](https://teamuchuaddon.github.io/UchuAddonWiki/inquiry/)、及び[Twitter(現X)](https://x.com/uchu_addons)のDMにお願いいたします。
---

## 概要

- **種別**：Addon
- **ベースMOD**：Nebula on the Ship

---

## 参考・影響を受けたMOD

本MODは、以下のMODを参考にしています。  
一部、役職アイデアやクレジット掲載の為ロゴを使用しています。  
以下のMod様に多大なる感謝を申し上げます。

- **[Nebula on the Ship](https://github.com/Dolly1016/Nebula)** （制作：Dolly）  
  本アドオンのベースとなるMOD
- **[Super New Roles](https://github.com/SuperNewRoles/SuperNewRoles)**（代表：ykundesu）  
  一部役職アイデア・ロゴを使用
- **[Extreme Roles](https://github.com/yukieiji/ExtremeRoles)**（制作：Yukieiji）  
  一部役職アイデア・ロゴを使用
- **[Town of Host_Y](https://github.com/Yumenopai/TownOfHost_Y)**（制作：Yumenopai）  
  一部役職アイデア・ロゴを使用
- **[Town of Host-K](https://github.com/KYMario/TownOfHost-K)**（制作：KYMario）  
  一部役職アイデア・ロゴを使用
- **[Town of Us](https://github.com/slushiegoose/Town-Of-Us)**（制作：slushiegoose）  
  一部役職アイデア・ロゴを使用
- **[Town of Us - Mira -](https://github.com/AU-Avengers/TOU-Mira)**（制作：AU Avengers）  
  一部役職アイデア・ロゴを使用
- **[The Other Roles](https://github.com/TheOtherRolesAU/TheOtherRoles)**（代表：Eisbison）  
  一部役職アイデア・ロゴを使用
- **[The Other Roles: GM Edition](https://github.com/yukinogatari/TheOtherRoles-GM)**（代表：yukinogatari）  
  一部役職アイデア・ロゴを使用
- **[BetterPolus](https://github.com/Brybry16/BetterPolus)**（製作：Brybry16）  
  Polusのマップ改造機能の参考元です。

---

## Wiki（仕様・役職一覧）

役職の仕様やシステムの詳細は Wiki にまとめています。  
質問・確認はこちらを参照してください。

👉 [UchuAddon Wiki](https://TeamUchuaddon.github.io/UchuAddonWiki/)

---

## その他SNSリンク
UchuAddonの情報を発信している公式アカウントです。
- ## [Twitter(現X)](https://x.com/uchu_addons)
  - UchuAddonの情報を不定期に発信します。  
  また、当アドオンについてお問い合わせがある場合DMにて対応いたします。
- ## [YouTube](https://www.youtube.com/@TeamUchuaddon)
  - UchuAddonの情報及び動画を不定期に投稿します。

---

## 移植・二次利用に関するガイドライン

本アドオン（UchuAddon）の移植および二次利用については、以下のガイドラインを遵守してください。

### 1. 役職アイディアおよび仕様について
**【移植の許可】** UchuAddonが独自に考案した役職のアイディア・ゲーム性・ロジックに関しては、著作権を主張いたしません。他環境への移植や、改変した役職の作成は自由に行っていただいて構いません。

> [!IMPORTANT]
> ただし、他MOD由来の役職については本項は適用されません。詳細は「3. 他MOD由来の役職について」をご確認ください。

### 2. 独自リソースの利用禁止
**【禁止事項】** アドオン内に含まれる以下のリソースを、そのまま（あるいは改変して）抜き出し、他MODやプロジェクトへ移植・利用することは**一切禁止**いたします。

* ボタンイラスト
* 画像・テクスチャ
* エフェクト
* その他各種リソース

### 3. 他MOD由来の役職について
**【遵守事項】** 本アドオンに実装されている他MOD由来の役職に関しては、それぞれの元MODが定める規約およびガイドラインに従ってください。

**【免責事項】** 他MODの役職移植に関するトラブルや権利侵害について、本アドオン制作側は一切の責任を負いません。

---

## Copyright

This mod is not affiliated with Among Us or Innersloth LLC, and the content contained therein is not endorsed or otherwise sponsored by Innersloth LLC. Portions of the materials contained herein are property of Innersloth LLC.  
© Innersloth LLC.
